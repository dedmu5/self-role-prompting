...
def main():
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError("OPENAI_API_KEY is not set. Export it before running experiments.")
    ...
    for i, data in enumerate(dataloader):
        ...
        if args.method == "adaptive_role_play":
            input_role_selection = (
                "Choose your role for the following question/task:\n"
                + x
                + "\nRemember, just respond choosing your role, not answering the question."
            )
            z = decoder.decode(args, input_role_selection, role_selection=True)
            args.role_setting = input_role_selection
            args.reply = z
        else:
            z = decoder.decode(args, x)

        # The final prompt combines the original task, intermediate response, and answer trigger.
        z2 = x + "\n" + z + args.direct_answer_trigger
        pred = decoder.decode(args, z2, role_selection=False)
        print(z2 + pred)
